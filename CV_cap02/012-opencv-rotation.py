...
# python 012-opencv-rotation.py --image images/ferrari.jpg
...
print('Shape da imagem:', image.shape[:2])
(h, w) = image.shape[:2]
center = (w // 2, h // 2)
print(f"h: {h}; w: {w}")
...
M = cv2.getRotationMatrix2D(center, -90, 1.0) # Cuidado com o sinal negativo
rotated = cv2.warpAffine(image, M, (w, h))
